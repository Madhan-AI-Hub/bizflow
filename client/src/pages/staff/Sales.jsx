...
const StaffSales = () => {
  ...
  const handleUpdatePayment = async () => {
    try {
      ...
      showToast(t('updatePaymentSuccess'));
  ...
  return (
    <AppLayout>
      ...
      {loading ? (<Box display="flex" justifyContent="center" py={4}><CircularProgress /></Box>) : sales.length === 0 ? (<Card><EmptyState title={t('noSalesYet')} message={t('addYourFirstProduct')} actionLabel={t('createSale')} onAction={() => setDialogOpen(true)} /></Card>) : (
        <Card>
          <TableContainer>
            <Table>
              ...
              <TableBody>
                {sales.map((sale) => (
                  <TableRow key={sale._id} hover>
                    ...
                    <TableCell>
                      <Chip 
                        label={t(sale.paymentStatus?.toLowerCase() || 'paid')} 
                        size="small" 
            ...
            <Grid item xs={12}>
              ...
              {selectedItems.map((item, index) => (
                <Box key={index} display="flex" gap={1} mb={1}>
                  <Autocomplete sx={{ flex: 1 }} options={products} getOptionLabel={(option) => `${option.name} - ₹${option.price}`} value={item.product} onChange={(e, value) => updateItem(index, 'product', value)} renderInput={(params) => <TextField {...params} label={t('product')} size="small" />} />
      ...
      <Dialog open={editDialogOpen} onClose={() => setEditDialogOpen(false)} maxWidth="xs" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="contained" onClick={handleUpdatePayment} disabled={paymentAmount <= 0}>{t('updatePayment')}</Button>
        </DialogActions>
      </Dialog>
    </AppLayout>
  );
